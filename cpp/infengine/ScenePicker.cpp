 ...
 *   - Gizmo axis handles (translate/rotate/scale) are picked via a dedicated
 *     lightweight `PickGizmoAxis()` that only tests 3 axes — no scene query.
 *
 * Contains: CollectIconHits,
 *           InfEngine::PickSceneObjectId, InfEngine::PickSceneObjectIds,
 *           InfEngine::PickGizmoAxis,
...
static void CollectIconHits(InfRenderer *rendererPtr, const glm::vec3 &rayOrigin, const glm::vec3 &rayDirection,
...
/// Internal: Test gizmo axis proximity against a ray.  Returns axis ID or 0.
...
uint64_t InfEngine::PickGizmoAxis(float screenX, float screenY, float viewportWidth, float viewportHeight)
{
