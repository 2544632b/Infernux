                                          ...
                                          int total_amount)
{
    ...
    if (!SDL_PutAudioStreamData(stream, output.data(), requestedFrames * bytesPerFrame)) {
        INFLOG_WARN("AudioEngine: failed to push streamed audio data: ", SDL_GetError());
    }
}
...
SDL_AudioStream *AudioEngine::CreateVoice(AudioSource * /*source*/, AudioClip *clip)
{
    ...
    if (!SDL_ConvertAudioSamples(&sourceSpec, clipData.data(), static_cast<int>(clipData.size()), &playbackSpec,
        ...
        INFLOG_ERROR("Failed to convert audio clip for playback: ", SDL_GetError());
        return nullptr;
    }
    ...
    if (!SDL_SetAudioStreamGetCallback(stream, &AudioEngine::FeedVoiceStream, voiceState.get())) {
        INFLOG_ERROR("Failed to register audio stream callback: ", SDL_GetError());
        SDL_DestroyAudioStream(stream);
        return nullptr;
    }
    ...
    if (!SDL_ResumeAudioStreamDevice(stream)) {
        INFLOG_ERROR("Failed to resume audio stream device: ", SDL_GetError());
        SDL_UnbindAudioStream(stream);
        SDL_DestroyAudioStream(stream);
        return nullptr;
    }
...
void AudioEngine::RegisterListener(AudioListener *listener)
{
    ...
    INFLOG_INFO("AudioListener: only one listener can be active. Keeping '", activeName, "' active; listener '",
...
void AudioEngine::UnregisterListener(AudioListener *listener)
{
    ...
    if (promoted) {
        ...
        INFLOG_INFO("AudioListener: active listener changed. Promoting standby listener '", promotedName, "'.");
    } else if (lostActiveListener) {
        INFLOG_INFO("AudioListener: there is currently no active listener in the scene.");
    }
}
