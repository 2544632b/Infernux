                ...
                if (tj.contains("clip_guid")) {
                    ...
                    if (!clipGuid.empty() && registry.IsInitialized()) {
                        ...
                        if (clip) {
                            ...
                            INFLOG_WARN("AudioSource::Deserialize: failed to load clip GUID for track ", i, ": ",
                                        clipGuid);
                        }
                    }
                }
...
{
    if (count < 1) {
        INFLOG_WARN("AudioSource::SetTrackCount: track_count must be >= 1. Clamping ", count, " to 1.");
        count = 1;
    }
...
void AudioSource::SetTrackVolume(int trackIndex, float volume)
{
    ...
    if (clamped != volume) {
        INFLOG_DEBUG("AudioSource::SetTrackVolume: volume must be in [0, 1]. Clamping ", volume, " to ", clamped, ".");
    }
...
void AudioSource::PlayOneShot(std::shared_ptr<AudioClip> clip, float volumeScale)
{
    if (!clip || !clip->IsLoaded()) {
        INFLOG_WARN("AudioSource::PlayOneShot: clip is null or not loaded.");
        return;
    }

    const float clampedVolumeScale = std::clamp(volumeScale, 0.0f, 1.0f);
    if (clampedVolumeScale != volumeScale) {
        INFLOG_DEBUG("AudioSource::PlayOneShot: volumeScale must be in [0, 1]. Clamping ", volumeScale, " to ",
                     clampedVolumeScale, ".");
    }
    ...
    if (selectedVoice < 0) {
        INFLOG_WARN("AudioSource::PlayOneShot: one-shot pool is empty after allocation attempt.");
        return;
    }

    if (m_oneShotVoices[selectedVoice].stream) {
        INFLOG_WARN("AudioSource::PlayOneShot: one-shot pool exhausted. Reusing the oldest pooled voice.");
...
void AudioSource::SetVolume(float volume)
{
    const float clamped = std::clamp(volume, 0.0f, 1.0f);
    if (clamped != volume) {
        INFLOG_DEBUG("AudioSource::SetVolume: volume must be in [0, 1]. Clamping ", volume, " to ", clamped, ".");
    }
    m_volume = clamped;
    ApplyAllTrackGains();
}
...
void AudioSource::SetPitch(float pitch)
{
    ...
    if (clamped != pitch) {
        INFLOG_DEBUG("AudioSource::SetPitch: pitch must be in [0.1, 3.0]. Clamping ", pitch, " to ", clamped, ".");
    }
    m_pitch = clamped;
    ApplyAllTrackGains();
}
...
void AudioSource::SetMinDistance(float dist)
{
    ...
    if (clamped != dist) {
        INFLOG_DEBUG("AudioSource::SetMinDistance: min_distance must be > 0. Clamping ", dist, " to ", clamped, ".");
    }
...
void AudioSource::SetMaxDistance(float dist)
{
    ...
    if (clamped != dist) {
        INFLOG_DEBUG("AudioSource::SetMaxDistance: max_distance must be >= min_distance. Clamping ", dist, " to ",
                     clamped, ".");
    }
...
{
    if (size < 1) {
        INFLOG_DEBUG("AudioSource::SetOneShotPoolSize: pool size must be >= 1. Clamping ", size, " to 1.");
        size = 1;
    }
...
void AudioSource::StartOneShotVoice(int voiceIndex, std::shared_ptr<AudioClip> clip, float volumeScale)
{
    auto &engine = AudioEngine::Instance();
    if (!engine.IsInitialized()) {
        INFLOG_WARN("AudioSource::StartOneShotVoice: AudioEngine not initialized");
        return;
    }
    ...
    if (!voice.stream) {
        INFLOG_ERROR("AudioSource::StartOneShotVoice: failed to create pooled voice ", voiceIndex);
        voice.clip.reset();
