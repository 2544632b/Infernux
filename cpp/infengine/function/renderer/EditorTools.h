 ...
 *
 * Supported modes:
 *  - **Translate** (W): three arrows (cylinder + cone), one per axis.
 *  - **Rotate**    (E): three torus rings, one per axis.
 *  - **Scale**     (R): three lines with cube endpoints, one per axis.
 *
...
class EditorTools
{
  public:
    ...
    /// Which axis handle is being hovered/dragged
    ...
    /// Set the highlighted (hovered) axis and rebuild mesh vertex colours.
    /// @param axis None / X / Y / Z
     ...
     */
    [[nodiscard]] DrawCallResult GetDrawCalls(std::shared_ptr<InfMaterial> material, uint64_t selectedObjId,
                                              Scene *activeScene, const glm::vec3 &cameraPos);

    // ====================================================================
    // Gizmo axis object IDs — used by both C++ and Python for identification
    // ====================================================================
