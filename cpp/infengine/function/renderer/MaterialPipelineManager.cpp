...
VkRenderPass MaterialPipelineManager::GetActiveMRTRenderPass()
{
    ...
    if (vkCreateRenderPass(m_device, &rpInfo, nullptr, &rp) != VK_SUCCESS) {
        INFLOG_ERROR("MaterialPipelineManager: Failed to create MRT render pass (", m_activeColorAttachmentCount,
                     " color attachments)");
        return m_internalRenderPass; // fallback
    }

    m_mrtRenderPassCache[key] = rp;
    INFLOG_INFO("MaterialPipelineManager: Created MRT render pass with ", m_activeColorAttachmentCount,
                " color attachments");
    return rp;
}
...
void MaterialPipelineManager::InvalidateAllMaterialPipelines()
{
    ...
    if (count > 0) {
        INFLOG_INFO("InvalidateAllMaterialPipelines: marked ", count, " materials dirty");
    }
}
