...
void TransientResourcePool::Release(uint32_t slotId)
{
    if (slotId >= m_entries.size()) {
        INFLOG_WARN("TransientResourcePool::Release — invalid slotId ", slotId);
        return;
    }

    auto &entry = m_entries[slotId];
    if (!entry.inUse) {
        INFLOG_WARN("TransientResourcePool::Release — slot ", slotId, " is not in use");
        return;
    }
...
uint32_t TransientResourcePool::CreateEntry(int width, int height, VkFormat format, VkSampleCountFlagBits samples)
{
    ...
    if (allocResult != VK_SUCCESS) {
        INFLOG_ERROR("TransientResourcePool: Failed to create VkImage (", width, "x", height, ")");
        return UINT32_MAX;
    }
    ...
    if (vkCreateImageView(device, &viewInfo, nullptr, &view) != VK_SUCCESS) {
        INFLOG_ERROR("TransientResourcePool: Failed to create VkImageView");
