...
void ShaderCache::Initialize(const std::string &cacheDirectory)
{
    ...
    if (!fs::exists(m_cacheDirectory)) {
        ...
        INFLOG_INFO("ShaderCache: Created cache directory: ", m_cacheDirectory);
    }
    ...
    m_initialized = true;
    INFLOG_INFO("ShaderCache: Initialized with ", m_cache.size(), " cached entries");
}
...
void ShaderCache::Shutdown()
{
    ...
    INFLOG_INFO("ShaderCache: Shutdown complete");
}
...
void ShaderCache::Store(const ShaderCacheKey &key, const std::vector<uint32_t> &spirv)
{
    ...
    INFLOG_DEBUG("ShaderCache: Stored shader '", key.shaderPath, "' with ", key.enabledKeywords.size(), " keywords");
}
...
void ShaderCache::ClearCache()
{
    ...
    INFLOG_INFO("ShaderCache: Cache cleared");
}
...
{
    if (!fs::exists(shaderDirectory)) {
        INFLOG_WARN("ShaderCache::PrecompileShaders: Directory does not exist: ", shaderDirectory);
        return;
    }

    INFLOG_INFO("ShaderCache: Precompiling shaders in ", shaderDirectory);
    ...
    for (const auto &entry : fs::recursive_directory_iterator(shaderDirectory)) {
        ...
        if (!file.is_open()) {
            INFLOG_WARN("ShaderCache: Failed to open shader: ", shaderPath);
            continue;
        }
        ...
        if (HasCached(key)) {
            INFLOG_DEBUG("ShaderCache: Shader already cached: ", shaderPath);
            continue;
        }
        ...
        try {
            ...
            if (!spirv.empty()) {
                Store(key, spirv);
                INFLOG_INFO("ShaderCache: Precompiled shader: ", shaderPath);
            }
        } catch (const std::exception &e) {
            INFLOG_ERROR("ShaderCache: Failed to precompile shader '", shaderPath, "': ", e.what());
        }
    }
    ...
    INFLOG_INFO("ShaderCache: Precompilation complete, ", m_cache.size(), " shaders cached");
}
...
fs::path ShaderCache::GetCacheFilePath(const ShaderCacheKey &key) const
{
    ...
    ss << std::hex << hash << ".spv";
    return fs::path(m_cacheDirectory) / ss.str();
}
...
void ShaderCache::SaveToDisk(const fs::path &path, const CachedSpirv &cached) const
{
    ...
    if (!file.is_open()) {
        INFLOG_WARN("ShaderCache: Failed to save cache to ", path.string());
        return;
    }
...
{
    fs::path indexPath = fs::path(m_cacheDirectory) / "cache_index.json";
    ...
    try {
        ...
        for (const auto &entry : j["entries"]) {
            ...
            fs::path spvPath = fs::path(m_cacheDirectory) / spvFile;
        ...
        INFLOG_WARN("ShaderCache: Failed to load cache index: ", e.what());
    }
}
...
void ShaderCache::SaveCacheIndex()
{
    ...
    try {
        ...
        INFLOG_WARN("ShaderCache: Failed to save cache index: ", e.what());
    }
}
