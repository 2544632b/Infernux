...
bool ShaderReflection::Reflect(const std::vector<char> &spirvCode, VkShaderStageFlagBits stage)
{
    if (spirvCode.empty() || spirvCode.size() % 4 != 0) {
        INFLOG_ERROR("Invalid SPIR-V code size: ", spirvCode.size());
        return false;
    }
...
bool ShaderReflection::Reflect(const std::vector<uint32_t> &spirvCode, VkShaderStageFlagBits stage)
{
    ...
    if (spirvCode.empty()) {
        INFLOG_ERROR("Empty SPIR-V code");
        return false;
    }
    ...
    try {
        ...
        for (const auto &ubo : resources.uniform_buffers) {
            ...
            INFLOG_DEBUG("Reflected UBO: ", info.name, " binding=", info.binding, " set=", info.set,
                         " size=", info.size);
        }
        ...
        for (const auto &image : resources.sampled_images) {
            ...
            INFLOG_DEBUG("Reflected sampled image: ", info.name, " binding=", info.binding, " set=", info.set);
        }
        ...
        for (const auto &sampler : resources.separate_samplers) {
            ...
            m_sampledImages.push_back(info);
            INFLOG_DEBUG("Reflected separate sampler: ", info.name, " binding=", info.binding);
        }
        ...
        for (const auto &image : resources.separate_images) {
            ...
            m_sampledImages.push_back(info);
            INFLOG_DEBUG("Reflected separate image: ", info.name, " binding=", info.binding);
        }
        ...
        for (const auto &pc : resources.push_constant_buffers) {
            ...
            INFLOG_DEBUG("Reflected push constant: ", info.name, " size=", info.size);
        }
        ...
        INFLOG_DEBUG("Shader reflection complete: ", m_uniformBuffers.size(), " UBOs, ", m_sampledImages.size(),
                     " samplers");

        return true;
    } catch (const std::exception &e) {
        INFLOG_ERROR("SPIRV-Cross reflection failed: ", e.what());
        return false;
    }
}
