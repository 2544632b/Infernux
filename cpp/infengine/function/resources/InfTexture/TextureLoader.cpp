...
// =============================================================================
// Load — create an InfTexture with import settings from .meta
// =============================================================================
...
std::shared_ptr<void> TextureLoader::Load(const std::string &filePath, const std::string &guid, AssetDatabase * /*adb*/)
{
    if (filePath.empty() || guid.empty()) {
        INFLOG_WARN("TextureLoader::Load: empty filePath or guid");
        return nullptr;
    }

    auto texture = std::make_shared<InfTexture>();
    ...
    INFLOG_INFO("TextureLoader: loaded '", texture->GetName(), "' (GUID: ", guid,
...
bool TextureLoader::Reload(std::shared_ptr<void> existing, const std::string &filePath, const std::string &guid,
                           AssetDatabase * /*adb*/)
{
    auto tex = std::static_pointer_cast<InfTexture>(existing);
    if (!tex) {
        INFLOG_WARN("TextureLoader::Reload: null existing instance");
        return false;
    }
    ...
    INFLOG_INFO("TextureLoader: reloaded '", tex->GetName(), "' in-place (GUID: ", guid, ")");
    return true;
}
