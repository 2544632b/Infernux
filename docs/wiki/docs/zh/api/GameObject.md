...
| is_static | `bool` | 静态标志。 |
| scene | `Optional[Scene]` | 此 GameObject 所属的场景。 *(只读)* |
...
| `add_component(component_type: Union[str, type]) → Optional[Component]` | 通过类型或类型名称添加 C++ 组件。 |
...
| `add_py_component(component_instance: Any) → Optional[Any]` | 向此 GameObject 添加 Python InfComponent 实例。 |
| `get_py_component(component_type: type) → Optional[Any]` | 获取指定类型的 Python 组件。 |
...
| `get_child(index: int) → Optional[GameObject]` | 根据索引获取子对象。 |
