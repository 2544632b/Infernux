...
| local_forward | `Vector3` | Forward direction in local space (negative Z). *(只读)* |
| local_right | `Vector3` | Right direction in local space (positive X). *(只读)* |
| local_up | `Vector3` | Up direction in local space (positive Y). *(只读)* |
| rotation | `Tuple[float, float, float, float]` | 世界空间中的旋转（四元数）。 |
| local_rotation | `Tuple[float, float, float, float]` | 相对于父变换的旋转。 |
...
| `get_child(index: int) → Optional[Transform]` | 按索引获取子变换。 |
